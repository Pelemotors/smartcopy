      ...
      try {
        ...
        if (response.ok) {
          ...
          setFormData({
            ...
            features_he: program.features_he || [],
            ...
            includes_whatsapp: program.includes_whatsapp || false,
            image_url: program.image_url || '',
            icon_url: program.icon_url || '',
            display_order: program.display_order ? String(program.display_order) : '0',
            active: program.active !== undefined ? program.active : true,
          });
        } else {
          setErrors({ submit: 'שגיאה בטעינת המסלול' });
        }
      } catch (error) {
        setErrors({ submit: 'שגיאה כללית בטעינת המסלול' });
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setErrors({ submit: data.error || 'שגיאה בעדכון המסלול' });
      }
    } catch (error) {
  ...
  if (isLoading) {
    return (
      <div className="container mx-auto px-4 py-8 text-center">
        <p className="text-text-dark/60 font-body">טוען מסלול...</p>
      </div>
    );
  }
